...
public:
  ...
  int diameter(){
    ...
    for (int i = 1; i <= n; i++){
      for (int d = i+1; d <= n; d++){
        if (distance(i,d)>max){max=distance(i,d);}
        if (distance(i,d)==-1){return -1;}
      }
    }
    return max;
  }
  ...
  void dfs_scc(int v) {
    nodes[v].num = nodes[v].low = dfsIndex++;
    ...
    for (auto e : nodes[v].adj) {
      ...
      if (nodes[w].num == -1) {
        dfs_scc(w);
        if (nodes[w].low < nodes[v].low)
          nodes[v].low = nodes[w].low;
      }
      else if (nodes[w].inStack) {
        if (nodes[w].num < nodes[v].low)
          nodes[v].low = nodes[w].num;
      }
    }
